...
# Only Store the Values we care about
                ...
                if True:
                    ...
                    # and int(year) > 1860 :
                    if ((int(year) == 1861 and int(month) == 12) or
                            (int(year) == 2862 and int(month) < 7)):

                        dict2 = {"recType": recType, "statID": statID, "statName": statName,
                                 "metElType": metElType, "metUnits": metUnits, "year": year,
                                 "month": month,  "timeInterval": timeInterval,
                                 "readingCt": readingCt,  "fullRow": thisRow}
                        ...
                        for obs in range(0, readingCt):
                            ...
                            dict1 = {"recType": recType, "statID": statID, "statName": statName,
                                     "metElType": metElType, "metUnits": metUnits, "year": year,
                                     "month": month,  "timeInterval": timeInterval,
                                     "readingCt": readingCt, "obsDayOfMonth": obsDayOfMonth,
                                     "obsHour": obsHour,  "obsValue": obsValue,  "obsQC1": obsQC1,
                                     "obsQC2": obsQC2, "AECode": AECode, "afterSlash": afterSlash,
                                     "fullRow": thisRow}
...
df = pd.DataFrame(columns=['recType', 'statID', 'statName', 'metElType', 'metUnits', 'year', 'month',
                           'timeInterval', 'filler99readingCt', 'obsDayOfMonth', 'obsHour', 'obsSign',
                           'obsValue', 'obsQC1', 'obsQC2', 'AECode', 'afterSlash', 'fullRow'])

df = pd.DataFrame(readinglist)
readingListColumnsTitles = ["statID", "statName", "recType", "metElType", "metUnits",
                            "timeInterval",  "readingCt", "year",  "month",  "obsDayOfMonth",
                            "obsHour",  "obsValue",  "obsQC1",  "obsQC2", "AECode", "afterSlash",
                            "fullRow"]
df = df.reindex(columns=readingListColumnsTitles)
df.to_csv("ParsedDataSHORT-FDP.csv", sep=',')

df_rows = pd.DataFrame(columns=['recType', 'statID', 'statName', 'metElType', 'metUnits', 'year',
                                'month', 'timeInterval', 'filler99readingCt', 'FullRow'])
df_rows = pd.DataFrame(rowList)
rowListColumnsTitles = ["statID", "statName", "recType", "metElType", "metUnits",   "timeInterval",
